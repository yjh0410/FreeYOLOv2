...
class YOLOv4(nn.Module):
    def __init__(self, 
        ...
        # --------- Network Parameters ----------
        ## backbone
        self.backbone, feat_dims = build_backbone(cfg=cfg, trainable=trainable)

        ## neck
        self.neck = build_neck(cfg=cfg, in_dim=feat_dims[-1], out_dim=feat_dims[-1])
        feat_dims[-1] = self.neck.out_dim
        
        ## fpn
        self.fpn = build_fpn(cfg=cfg, in_dims=feat_dims, out_dim=cfg['head_dim'])
